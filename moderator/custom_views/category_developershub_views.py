...
def new_developers_hub_content_from_template(request):
    ...
    if page:
        context['blogs'] = blog_content_init.get_content(context="programming", tag="programming, developer", page=page)
    else:
        context['blogs'] = blog_content_init.get_content(context="programming", tag="programming, developer")

    return render(request, 'moderator/categories/developershub/new_content_template.html', context=context)
...
def process_new_developers_hub_content_from_template(request):
    if request.method == 'POST':
        ...
        if blog_object:
            ...
            request.session["new_blog_save_status"] = False
    return redirect(reverse('shadownetz_moderator:category_developershub_add_template'))
