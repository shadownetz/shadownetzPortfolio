...
from .models import (TrendingBlog, TrendingBlogComment)
from django.core.exceptions import ObjectDoesNotExist


class TrendingBlogClass:

    def __init__(self):
        self.blog_model = TrendingBlog
    ...
    def get_blog_content(self, page, per_page_number):
        TrendingBlogs = self.get_model_content()
        paginator = Paginator(TrendingBlogs, per_page_number)
        blogs = ''
    ...
    def post_comment(self, blog_id, email, message):
        if blog_id and email and message:
            ...
            if main_blog:
                tmp_object = TrendingBlogComment.objects.create(
    ...
    def get_comments(self, blog_id):
        ...
        if blog_id:
            main_blog = self.get_single_blog_content(blog_id=blog_id)
            if main_blog:
                selected_comments = TrendingBlogComment.objects.filter(blog=main_blog).order_by('id')[::-1]
    ...
    def get_comment_greater_than_id(self, blog_id, tmp_id):
        if blog_id:
            ...
            main_blog = self.get_single_blog_content(blog_id=blog_id)
            if main_blog:
                selected_comments = TrendingBlogComment.objects.filter(blog=main_blog, id__gt=last_comment_id)
    ...
    def get_total_no_of_comment(self, blog_id):
        if blog_id:
            main_blog = self.get_single_blog_content(blog_id=blog_id)
            if main_blog:
                return TrendingBlogComment.objects.filter(blog=main_blog).count()
        return 0
    ...
    def filter_blog_content_by_search(self, query, page, per_page_number, context="title"):
        ...
        """
        if context == 'title':
            TrendingBlogs = self.filter_model_content(title=query)
        else:
            TrendingBlogs = self.filter_model_content()
